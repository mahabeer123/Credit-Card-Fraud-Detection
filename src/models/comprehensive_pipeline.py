...
Follows the complete ML workflow: EDA → Cleaning → Feature Selection → Scaling → SMOTE → Training → Testing → Comparison → SHAP
"""
...
class FraudDetectionPipeline:
    ...
    def step5_handle_class_imbalance(self):
        """Step 5: Handle Class Imbalance with SMOTE - Balance to 50:50"""
        print("\n" + "="*60)
        print("⚖️ STEP 5: HANDLING CLASS IMBALANCE WITH SMOTE (50:50)")
        print("="*60)
        ...
        # Apply SMOTE to balance to 50:50
        print("\nApplying SMOTE to balance classes to 50:50...")
    ...
    def step8_compare_with_without_distance(self):
        ...
        if 'distance' in self.X_train.columns:
            ...
            rf_without = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
            rf_with = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
    ...
    def run_complete_pipeline(self):
        ...
        print("="*60)
        print("Following steps: EDA → Cleaning → Feature Selection → Scaling → SMOTE → Training → Testing → Comparison → SHAP")
        print("="*60)
